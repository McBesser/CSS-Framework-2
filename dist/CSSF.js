...
(function() {
     ...
     getStyles(cssClasses) {
        ...
        cssClasses.forEach((cssClass, cssClassIndex) => {
           ...
           cssClass.split('--').forEach((part, partIndex) => {
              const aliasesSubParts = part.split('_');
              const aliasesName = aliasesSubParts.shift();
              if (this.aliases[aliasesName]) {
                 const aliasesRawData = this.aliases[aliasesName];
                 const aliasesConvert = this.fillaliases(aliasesRawData, aliasesSubParts);
                 parts.push(...aliasesConvert.split('--'));
              } else {
                 parts.push(...[part]);
              }
           });
  ...
  // Expose the instance and classes to the global scope
  window.cssfInstance = cssfInstance;
  window.CSSF = CSSF;
